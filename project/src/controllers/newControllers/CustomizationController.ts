...
export class CustomizationController {
    ...
    public getCustomisationUnlocks(
        ...
        const data = this.saveServer.getProfile(sessionID).customisationUnlocks;
        return this.httpResponse.getBody(data);
    }
    ...
    public getTraderSuits(url: string, _info: IEmptyRequestData, sessionID: string): IGetBodyResponseData<ISuit[]> {
        ...
        const data = this.getTraderSuitsHelper(traderID, sessionID) ?? [];
        return this.httpResponse.getBody(data);
    }
    ...
    public buyCustomisation(
        ...
        return this.buyCustomisationHelper(pmcData, body, sessionID);
    }
    ...
    public getHideoutCustomisation(
        ...
        const data = this.databaseService.getHideout().customisation;
        return this.httpResponse.getBody(data);
    }
    ...
    public getStorage(
        ...
        if (!profile) {
            return this.httpResponse.getBody(customisationResultsClone);
        }
     ...
     * @returns ISuit array
     */
    private getTraderSuitsHelper(traderID: string, sessionID: string): ISuit[] | undefined {
     ...
     */
    private buyCustomisationHelper(
        pmcData: IPmcData,
    ...
    private getTraderClothingOffer(sessionId: string, offerId: string): ISuit {
     ...
     */
    private outfitAlreadyPurchased(suitId: string, sessionID: string): boolean {
    ...
    private getAllTraderSuits(sessionID: string): ISuit[] {
        const traders = this.databaseService.getTraders();
        let result: ISuit[] = [];

        for (const traderID in traders) {
            if (traders[traderID].base.customization_seller === true) {
                //May have borked this....
                result = [...result, ...(this.getTraderSuitsHelper(traderID, sessionID) ?? [])];
            }
        }
     ...
     */
    private applyClothingItemToProfile(customisation: CustomizationSetOption, pmcData: IPmcData): void {
